#!/usr/bin/env python3
#
# Tests the DNS configuration of a MeetrMail.
#
# tests/dns.py ipaddr hostname
#
# where ipaddr is the IP address of your MeetrMail
...
if not test(ipaddr, "MeetrMail"):
	print ()
	print (f"Please run the MeetrMail setup script on {hostname} again.")
	sys.exit(1)
else:
	print ("The MeetrMail provided correct DNS answers.")
	print ()
	...
	if not test("8.8.8.8", "Google Public DNS"):
		print ()
		print (f"Check that the nameserver settings for {hostname} are correct at your domain registrar. It may take a few hours for Google Public DNS to update after changes on your MeetrMail.")
		sys.exit(1)
	else:

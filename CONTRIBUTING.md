...
MeetrMail is an open source project. Your contributions and pull requests are welcome.

## Development

To start developing MeetrMail, [clone the repository](https://github.com/meetrmail/meetrmail) and familiarize yourself with the code.

    $ git clone https://github.com/meetrmail/meetrmail
...
With Vagrant set up, the following should boot up MeetrMail inside a virtual machine:
...
After a while, MeetrMail will be available at `192.168.56.4` (unless you changed that in your `Vagrantfile`). To be able to use the web-based bits, we recommend to add a hostname to your `hosts` file:

    $ echo "192.168.56.4 meetrmail.lan" | sudo tee -a /etc/hosts

You should now be able to navigate to https://meetrmail.lan/admin using your browser. There should be an initial admin user with the name `me@meetrmail.lan` and the password `12345678`.

### Making changes

Your working copy of MeetrMail will be mounted inside your VM at `/vagrant`. Any change you make locally will appear inside your VM automatically.
...
MeetrMail needs more tests. If you're still looking for a way to help out, writing and contributing tests would be a great start!

...
MeetrMail 1.0.0 is the first release of MeetrMail, forked from the Mail-in-a-Box v76 codebase.

* Ported from Ubuntu 22.04 to Ubuntu 24.04 LTS, with PHP 8.3, Python 3.12 via `uv`, Rspamd in place of SpamAssassin and postgrey, and Nextcloud 33. See FORK.md.
* Added sandbox mode and `meetrmail-mode` for installing and testing a box without a public internet presence.
* The version number is kept in the `VERSION` file and shown by setup, the control panel status checks and the API.
* Renamed the project to MeetrMail, an independent mail server stack based on Mail-in-a-Box v76. Paths, the `mailinabox` service, and the setup command (now `sudo meetrmail-setup`) were renamed; existing boxes are migrated automatically on the next setup run.

The entries below, from Version 76 back, are the release history of the upstream Mail-in-a-Box project.

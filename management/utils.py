...
# THE ENVIRONMENT FILE AT /etc/meetrmail.conf

def load_environment():
    # Load settings from /etc/meetrmail.conf.
    return load_env_vars_from_file("/etc/meetrmail.conf")
...
def save_environment(env):
    with open("/etc/meetrmail.conf", "w", encoding="utf-8") as f:
...
def get_php_version(env=None):
	"""The PHP version this box is configured for, as a string like "8.3".

	setup/start.sh records it as PHP_VERSION in /etc/meetrmail.conf. Fall back
	to looking for an installed php-fpm, so that code calling this still works on
	a box installed before that setting existed."""
	if env is None:
		env = load_environment()
	if env.get("PHP_VERSION"):
		return env["PHP_VERSION"]
	versions = sorted(os.path.basename(os.path.dirname(p)) for p in glob.glob("/etc/php/*/fpm/php.ini"))
	if not versions:
		msg = "Could not determine the PHP version this box uses: no /etc/php/*/fpm/php.ini and no PHP_VERSION in /etc/meetrmail.conf."

...
def get_web_domains(env, include_www_redirects=True, include_auto=True, exclude_dns_elsewhere=True):
	...
	if include_auto:
		...
		#
		# ('autodiscover.' was for Exchange ActiveSync autodiscovery, served by
		# Z-Push. Z-Push is gone, so we no longer serve or provision it.)
	...
	# and the control panel. This can't be removed by a custom A/AAAA
	# record and is never a 'www.' redirect.
...
def do_web_update(env):
	...
	#
	# The php-fpm socket path in nginx-top.conf carries the PHP version, so it
	# has to be substituted rather than used literally. v76 had "php8.0-fpm.sock"
	# hard-coded in the template, which silently breaks every PHP page on the box
	# -- webmail, Nextcloud, the control panel -- the moment PHP_VER changes.
	nginx_conf = read_conf("nginx-top.conf").replace("PHP_VERSION", get_php_version(env))

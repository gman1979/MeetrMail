#!/usr/local/lib/meetrmail/env/bin/python3
#
# The API can be accessed on the command line, e.g. use `curl` like so:
#    curl --user $(</var/lib/meetrmail/api.key): http://localhost:10222/mail/users
#
# During development, you can start the MeetrMail control panel
# by running this script, e.g.:
#
# service meetrmail stop # stop the system process
# DEBUG=1 management/daemon.py
# service meetrmail start # when done debugging, start it up again
...
def system_latest_upstream_version():
	from status_checks import get_latest_meetrmail_version
	try:
		# Returns None when no VERSION_CHECK_URL is configured, which is the
		# default for this fork. Flask cannot return None, so say so instead.
		return get_latest_meetrmail_version(env) or ""
	except Exception as e:
		return (str(e), 500)
...
def munin_cgi(filename):
	...
	the cgi script behind meetrmail's auth mechanisms and avoids additional
...
def log_failed_login(request):
	...
	# message.
	app.logger.warning("MeetrMail Management Daemon: Failed login attempt from ip %s - timestamp %s", ip, time.time())

...
def scan_mail_log(env):
    ...
    Args:
        env (dict): Dictionary containing MeetrMail settings
    ...
    collector = {
        ...
        "greylisted": {},  # Data about greylisting of email addresses
        "rejected": OrderedDict(),  # Emails that were blocked
        "known_addresses": None,  # Addresses handled by the MeetrMail installation
        "other-services": set(),
    }
    ...
    if collector["greylisted"]:
        ...
        print(textwrap.fill(
            ...
            "Legitimate senders must try again after five minutes.",
            width=80, initial_indent=" ", subsequent_indent=" "
        ), end='\n\n')

        data = OrderedDict(sorted(collector["greylisted"].items(), key=email_sort))
        users = []
        received = []
        senders = []
        sender_clients = []
...
def scan_mail_log_line(line, collector):
    ...
    if service == "postfix/submission/smtpd":
        ...
        if SCAN_GREY:
            scan_rspamd_line(date, log, collector)
    elif service == "postfix/smtpd":
        if SCAN_BLOCKED:
            scan_postfix_smtpd_line(date, log, collector)
    elif service in {"postfix/qmgr", "postfix/pickup", "postfix/cleanup", "postfix/scache",
                     "postfix/anvil", "postfix/master", "opendkim", "opendmarc",
                     "postfix/lmtp", "postfix/tlsmgr", "anvil", "dovecot"}:
        # nothing to look at
        return True
    else:
...
# Rspamd writes one summary line per message from rspamd_task_write_log, in the
# form:
#
#   <digest>; task; rspamd_task_write_log: id=<msgid>, qid=<queue-id>,
#     ip=1.2.3.4, from=<a@example.com>, (default: F (greylist): [4.20/15.00]
#     [SYMBOL1,SYMBOL2]), len=..., time=..., dns req=..., digest=<...>,
#     rcpts=<b@example.com>
#
# That single line carries everything the greylisting report needs, so unlike
# postgrey -- which logged a separate action=greylist/action=pass pair -- we see
# one line per delivery attempt and record the first and most recent time we saw
# each sender/recipient pair deferred.
RSPAMD_LOG_RE = re.compile(
    r"rspamd_task_write_log:.*?"
    r"(?:\bip=(?P<ip>[^,]+), )?"
    r"(?:\buser=(?P<user>[^,]+), )?"
    r"\bfrom=<(?P<sender>[^>]*)>.*?"
    r"\(default: [A-Z] \((?P<action>[a-z ]+)\): \[(?P<score>-?[\d.]+)/(?P<limit>-?[\d.]+)\]"
    r".*?\brcpts=<(?P<rcpts>[^>]*)>"
)


def scan_rspamd_line(date, log, collector):
    """ Scan an Rspamd log line and extract greylisting data """

    m = RSPAMD_LOG_RE.search(log)
    if not m:
        return

    if m.group("action") != "greylist":
        # Only greylisting is reported here. Spam scoring is far better read in
        # Rspamd's own web interface, which shows the contributing symbols.
        return

    sender = m.group("sender") or "unknown"
    client = m.group("ip") or "unknown"

    # A message can have several recipients; report it against each of ours.
    for user in (r.strip() for r in m.group("rcpts").split(",")):
        if not user or not user_match(user):
            continue

        key = (client, sender)
        rep = collector["greylisted"].setdefault(user, {})

        # (first seen, most recently seen). The report renders these as the
        # time the mail was first deferred and when it was last attempted.
        first, _last = rep.get(key, (None, None))
        rep[key] = (first or date, date)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Scan the mail log files for interesting data. By default, this script "
                    ...
                    "re)written for the Mail-in-a-Box email server, on which MeetrMail is based. "

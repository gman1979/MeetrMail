...
import os, os.path, glob, pathlib, re, datetime, sys
...
def get_duplicity_additional_args(env):
	...
	if get_target_type(config) == 'rsync':
		...
		return [
			f"--ssh-options='-i /root/.ssh/id_rsa_meetrmail -p {port}'",
			f"--rsync-options='-e \"/usr/bin/ssh -oStrictHostKeyChecking=no -oBatchMode=yes -p {port} -i /root/.ssh/id_rsa_meetrmail\"'",
		]
	if get_target_type(config) == 's3':
...
def get_duplicity_env_vars(env):
	...
	# duplicity runs on the system python3 (it comes from the Ubuntu package),
	# not on the management daemon's environment. Its Backblaze B2 and Amazon
	# S3 backends need b2sdk and boto3, which we install into a separate
	# uv-managed environment rather than into the system site-packages --
	# noble's PEP 668 marker makes writing there an error. Put that
	# environment on duplicity's import path. See setup/management.sh.
	dup_env_path = pathlib.Path("/usr/local/lib/meetrmail/duplicity-env-path")
	if dup_env_path.exists():
		site_packages = glob.glob(os.path.join(dup_env_path.read_text(encoding="utf-8").strip(), "lib/python*/site-packages"))
		if site_packages:
...
def perform_backup(full_backup):
	...
	# The PHP version is read from the config rather than hard-coded. v76 had
	# "php8.0-fpm" written in here, which was right for v76 but silently stops
	# matching the moment PHP_VER changes -- and with quit=True that fails the
	# whole backup.
	service_command(f"php{get_php_version(env)}-fpm", "stop", quit=True)
	service_command("postfix", "stop", quit=True)
	service_command("dovecot", "stop", quit=True)
	# Rspamd and Redis replace the postgrey that v76 stopped here. Redis holds
	# the spam filter's Bayes tokens, fuzzy hashes and greylist records, and its
	# data directory is under STORAGE_ROOT (see setup/rspamd.sh), so stopping it
	# is what makes the snapshot consistent. Stop Rspamd first so nothing is
	# still writing to Redis. quit=False because a failure to stop the spam
	# filter should not abandon the backup.
	service_command("rspamd", "stop", quit=False)
	service_command("redis-server", "stop", quit=False)
	...
	try:
		...
		service_command("rspamd", "start", quit=False)
		service_command("dovecot", "start", quit=False)
		service_command("postfix", "start", quit=False)
		service_command(f"php{get_php_version(env)}-fpm", "start", quit=False)
...
def list_target_files(config):
	...
	if target.scheme == "rsync":
		...
		rsync_command = [ 'rsync',
					'-e',
					f'/usr/bin/ssh -i /root/.ssh/id_rsa_meetrmail -oStrictHostKeyChecking=no -oBatchMode=yes -p {port}',
					'--list-only',
					'-r',
					rsync_target.format(
						host=user_host,
		...
		if 'Permission denied (publickey).' in listing:
			...
			reason = ("Unknown error."
					...
					"from meetrmail sources to debug the issue.")
...
def get_backup_config(env, for_save=False, for_ui=False):
	...
	ssh_pub_key = os.path.join('/root', '.ssh', 'id_rsa_meetrmail.pub')

...
def get_ssl_certificates(env):
	...
	# cryptography deprecated the naive not_valid_before/not_valid_after
	# properties (and Python deprecated datetime.utcnow()) in favour of
	# timezone-aware equivalents. Using the aware pair keeps these comparisons
	# correct and stops every call flooding the log with warnings.
	import datetime
			...
			with tempfile.NamedTemporaryFile() as csr_file:
				...
				with tempfile.TemporaryDirectory() as d:
					...
					certbotret = subprocess.check_output([
						...
						"--non-interactive", # will fail if user hasn't registered during MeetrMail setup

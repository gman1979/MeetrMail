...
def get_services():
	return [
		...
		{ "name": "Spam Filter (Rspamd milter)", "port": 11332, "public": False, },
		{ "name": "Spam Filter Control (Rspamd)", "port": 11334, "public": False, },
		{ "name": "Redis (Rspamd storage)", "port": 6379, "public": False, },
		{ "name": "OpenDKIM", "port": 8891, "public": False, },
		{ "name": "OpenDMARC", "port": 8893, "public": False, },
		{ "name": "MeetrMail Management Daemon", "port": 10222, "public": False, },
...
def is_sandbox(env):
	"""True when this box was installed in sandbox mode.

	Sandbox boxes deliberately have no public DNS delegation, no PTR record and
	no path to port 25, so the checks that depend on those cannot pass. Reporting
	them as errors would bury the failures that actually matter under a screen of
	expected red, so they are reported as informational instead."""
	return env.get("SANDBOX_MODE") == "1"
...
def run_checks(rounded_values, env, output, pool, domains_to_check=None):
	...
	if is_sandbox(env):
		output.print_warning("""This box is in SANDBOX mode: it is set up for local testing and is not
			reachable from the public internet. Checks that require public DNS, a PTR record, a
			publicly-trusted TLS certificate or outbound port 25 are skipped below and noted as
			such. Run 'sudo meetrmail-mode live' to switch this box to live mode.""")
...
def run_system_checks(rounded_values, env, output):
	...
	check_meetrmail_version(env, output)
...
def run_network_checks(env, output):
	...
	if is_sandbox(env):
		output.print_warning("Outbound mail (SMTP port 25) not checked: this box is in sandbox mode.")
	else:
	...
	if is_sandbox(env):
		output.print_warning("Blocklist status not checked: this box is in sandbox mode.")
		return
...
def evaluate_spamhaus_lookup(lookupaddress, lookuptype, lookupdomain, output, zen):
	...
	if zen is None:
		...
		output.print_warning(f"""MeetrMail is configured to use a public DNS server. This is not supported by
...
def check_primary_hostname_dns(domain, env, output, dns_domains, dns_zonefiles):
	...
	if is_sandbox(env):
		output.print_warning("""Reverse DNS (PTR) not checked: this box is in sandbox mode. Setting the PTR record for
			this box's IP to its hostname is one of the things to do before going live -- Gmail and Outlook
			will reject or junk mail from a box without it.""")
	else:
...
def check_mail_domain(domain, env, output):
	...
	if dbl is None:
		...
		output.print_warning(f"MeetrMail is configured to use a public DNS server. This is not supported by spamhaus. Could not determine whether the domain {domain} is blacklisted.")
...
def check_ssl_cert(domain, rounded_time, ssl_certificates, env, output):
	...
	if cert_status == "OK":
		...
		if is_sandbox(env):
			output.print_warning("""The TLS (SSL) certificate for this domain is self-signed, which is expected in
				sandbox mode -- Let's Encrypt cannot issue a certificate for a box it cannot reach. A real
				certificate will be provisioned when this box goes live.""")
...
def what_version_is_this(env):
	# The release version is recorded in the VERSION file at the top of the
	# MeetrMail source tree, so it is known even without git or a .git directory.
	meetrmail_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
	with open(os.path.join(meetrmail_dir, "VERSION"), encoding="utf-8") as f:
		return f.read().strip()

def get_version_check_url(env):
	"""Where to look up the latest available version, or None.

	Upstream hard-codes https://mailinabox.email/setup.sh here. That is wrong for
	a fork: the tag in upstream's script will never match this branch, so the
	control panel would show a permanent "a new version is available" error that
	is not actionable and that trains people to ignore the status page.

	Set VERSION_CHECK_URL in /etc/meetrmail.conf to a URL whose content contains
	a `TAG=<version>` line, or that is just a version number -- the VERSION file
	served raw -- to turn version checking back on."""
	return env.get("VERSION_CHECK_URL") or None


def get_latest_meetrmail_version(env):
	# Fetches the configured URL and extracts the tag named in it to determine
	# the latest available version.
	from urllib.request import urlopen, HTTPError, URLError

	url = get_version_check_url(env)
	if not url:
		return None
	if not url.startswith(("http://", "https://")):
		# The URL comes from /etc/meetrmail.conf. Refuse anything that isn't
		# HTTP, so a typo (or a bad edit) can't turn this into a file:// read.
		return None

	try:
		body = urlopen(url + ("&" if "?" in url else "?") + "ping=1", timeout=5).read()  # noqa: S310 -- scheme checked above
	except (TimeoutError, HTTPError, URLError, ValueError):
		return None

	m = re.search(rb'TAG=(.*)', body)
	if m:
		return m.group(1).decode("utf8").strip()
	# A raw VERSION file: the whole body is the version number.
	m = re.fullmatch(rb'\s*(\d+\.\d+\.\d+)\s*', body)
	return m.group(1).decode("utf8") if m else None

def check_meetrmail_version(env, output):
	...
	if config.get("privacy", True):
		output.print_warning(f"You are running version MeetrMail {this_ver}. MeetrMail version check disabled by privacy setting.")
	elif not get_version_check_url(env):
		output.print_ok(f"You are running MeetrMail {this_ver}. Automatic version checking is off "
		                "(set VERSION_CHECK_URL in /etc/meetrmail.conf to enable it).")
	else:
		latest_ver = get_latest_meetrmail_version(env)

		if this_ver == latest_ver:
			output.print_ok(f"MeetrMail is up to date. You are running version {this_ver}.")
		elif latest_ver is None:
			output.print_warning(f"The latest MeetrMail version could not be determined. You are running version {this_ver}.")
		else:
			output.print_error(f"A new version of MeetrMail is available. You are running version {this_ver}. The latest version is {latest_ver}.")
...
def run_and_output_changes(env, pool):
	...
	cache_fn = "/var/cache/meetrmail/status_checks.json"

...
# on the MeetrMail control panel backend. The script
# reads /var/lib/meetrmail/api.key for the backend's
...
def mgmt(cmd, data=None, is_json=False):
	...
	try:
		...
		if e.code == 401:
			...
			print("The management daemon refused access. The API key file may be out of sync. Try 'service meetrmail restart'.", file=sys.stderr)
		elif hasattr(e, 'read'):
...
def setup_key_auth(mgmt_uri):
	with open('/var/lib/meetrmail/api.key', encoding='utf-8') as f:
		key = f.read().strip()

	auth_handler = urllib.request.HTTPBasicAuthHandler()
	auth_handler.add_password(
		realm='MeetrMail Management Server',
		uri=mgmt_uri,
		user=key,
		passwd='')

...
# virtualenv (/usr/local/lib/meetrmail/env). So only
...
def is_sandbox():
	...
	Read straight from /etc/meetrmail.conf rather than taking an env argument,
	because this module's validate_email() is called from setup/questions.sh
	before that file exists, as well as from the management daemon afterwards."""
	try:
		return utils.load_environment().get("SANDBOX_MODE") == "1"
	except FileNotFoundError:
		# Setup hasn't written /etc/meetrmail.conf yet. During setup the flag
		# is in the environment instead.
		return os.environ.get("MEETRMAIL_SANDBOX") == "1"
...
def validate_email(email, mode=None):
	...
	#
	# test_environment permits the special-use TLDs reserved by RFC 2606 --
	# .test, .example, .invalid, .localhost -- which email_validator rejects by
	# default. Those are exactly the names a sandbox box should be using, so
	# allow them when this box is in sandbox mode and not otherwise: on a live
	# box an address at one of those domains can never receive mail, and
	# refusing it is the right answer.
	try:
		validate_email_(email,
			allow_smtputf8=False,
			check_deliverability=False,
			test_environment=is_sandbox(),

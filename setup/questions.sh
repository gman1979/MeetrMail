...
if [ -z "${NONINTERACTIVE:-}" ]; then
	...
	# v76 installed email_validator into the system interpreter here with
	# `pip3 install`. On noble that aborts with
	# "error: externally-managed-environment" because pip 24.0 enforces PEP
	# 668. We validate the address with setup_python() instead, which runs
	# management/mailconfig.py in a cached uv environment. See setup/uv.sh.

	message_box "MeetrMail Installation" \
		"Hello and thanks for deploying a MeetrMail!
		\n\nI'm going to ask you a few questions.
		\n\nTo change your answers later, just run 'sudo meetrmail-setup' from the command line.
		\n\nNOTE: You should only install this on a brand new Ubuntu installation 100% dedicated to MeetrMail. MeetrMail will, for example, remove apache2."
fi
...
\n\nEmail Address:" \
		...
		while ! setup_python management/mailconfig.py validate-email "$EMAIL_ADDR"
		do
			input_box "Your Email Address" \
...
echo "MeetrMail Version: $(cat VERSION)"
echo

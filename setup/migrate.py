...
# Migrates any file structures, database schemas, etc. between versions of MeetrMail.
...
def migration_3(env):
	# Move the migration ID from /etc/meetrmail.conf to $STORAGE_ROOT/meetrmail.version
...
def run_migrations():
	if not os.access("/etc/meetrmail.conf", os.W_OK, effective_ids=True):
		print("This script must be run as root.", file=sys.stderr)
		sys.exit(1)

	env = load_environment()

	migration_id_file = os.path.join(env['STORAGE_ROOT'], 'meetrmail.version')
	migration_id = None
	...
	while True:
		...
		print()
		print("Running migration to MeetrMail #%d..." % next_ver)

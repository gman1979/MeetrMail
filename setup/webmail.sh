...
#
# Note: libmagic1 was renamed libmagic1t64 in noble by the 64-bit time_t
# transition. The old name still resolves via Provides:, but we name the real
# package so that an apt policy change can't quietly break this.
echo "Installing Roundcube (webmail)..."
apt_install \
	dbconfig-common \
	php"${PHP_VER}"-cli php"${PHP_VER}"-sqlite3 php"${PHP_VER}"-intl php"${PHP_VER}"-common php"${PHP_VER}"-curl php"${PHP_VER}"-imap \
	php"${PHP_VER}"-gd php"${PHP_VER}"-pspell php"${PHP_VER}"-mbstring php"${PHP_VER}"-xml libjs-jquery libjs-jquery-mousewheel \
	libmagic1t64 \
	sqlite3
...
<?php
/*
 * Do not edit. Written by MeetrMail. Regenerated on updates.
 */
\$config = array();
...
\$config['support_url'] = 'https://meetrmail.net/';
...
sed -i.meetrmailold 's/^[^#]\+.\+PRAGMA journal_mode = WAL.\+$/#&/' \

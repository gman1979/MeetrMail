...
# No third-party apt repositories.
#
# v76 pulled in ppa:ondrej/php (for PHP 8.0) and
# ppa:duplicity-team/duplicity-release-git. Neither is needed on 24.04:
#
#  * noble ships PHP 8.3 in main/universe, including every module we use
#    (cli fpm sqlite3 gd imap curl dev xml mbstring zip intl gmp bcmath apcu
#    imagick common pspell), and Nextcloud 33 and Roundcube 1.6 both support it.
#  * noble ships duplicity 2.1.4 in main.
#
# The only third-party apt source left on the box is the Rspamd project's own
# repository, added by setup/rspamd.sh with its signing key pinned by
# fingerprint and apt-pinned to the rspamd package alone. That one is a
# deliberate trade: universe's Rspamd is two majors behind and carries no
# security-update guarantee, and this is the component reading hostile input
# from the public internet. Set RSPAMD_PACKAGE_SOURCE=ubuntu to drop it too.
...
# Old kernels pile up over time and take up a lot of disk space, and because of MeetrMail
...
# * bind9-host, bind9-dnsutils: `host` and `dig`. setup/network-checks.sh needs
#   `host`, and it is the first thing anyone reaches for when the box's DNS
#   misbehaves. v76 installed bind9-host only from network-checks.sh, which
#   means a box that skipped those checks had no way to query DNS at all.

echo "Installing system packages..."
# Note: `ntp` is deliberately absent. On noble it is a transitional package
# that pulls in ntpsec, which fights systemd-timesyncd for control of the
# clock. MeetrMail never wrote /etc/ntp.conf, so nothing is lost --
# systemd-timesyncd keeps the time correct on its own.
apt_install python3 python3-dev python3-pip python3-setuptools \
	netcat-openbsd wget curl git sudo coreutils bc file \
	pollinate openssh-client unzip xz-utils \
...
#
# Some systems are missing /etc/timezone, which we cat into the Nextcloud
# config, so we need to set it to something. Daily cron tasks
...
if [ -z "${NONINTERACTIVE:-}" ]; then
	if [ ! -f /etc/timezone ] || [ -n "${FIRST_TIME_SETUP:-}" ]; then
		...
		# MeetrMail setup, run the interactive timezone configuration
		# tool.
...
if is_sandbox; then
	# Writing to /dev/urandom needs CAP_SYS_ADMIN and pollinate needs to reach
	# entropy.ubuntu.com. Neither is available in an unprivileged container, and
	# neither affects what we are testing.
	sandbox_skip "random number generator seeding (pollinate, /dev/urandom reseed)"
else
...
if [ ! -f /root/.ssh/id_rsa_meetrmail ]; then
	echo 'Creating SSH key for backup…'
	ssh-keygen -t rsa -b 2048 -a 100 -f /root/.ssh/id_rsa_meetrmail -N '' -q
fi
...
# download bind9 from.
# Turn off systemd-resolved's stub listener so it doesn't hold 127.0.0.53:53
# and compete with bind9. A machine without systemd-resolved installed has no
# stub listener to turn off, which is fine -- but don't crash on the missing
# config file.
if [ -f /etc/systemd/resolved.conf ]; then
	tools/editconf.py /etc/systemd/resolved.conf DNSStubListener=no
fi
if mountpoint -q /etc/resolv.conf; then
	# In a container /etc/resolv.conf is a bind mount from the host and cannot
	# be unlinked, so overwrite its contents in place instead of replacing it.
...
cat conf/fail2ban/jails.conf \
	...
	> /etc/fail2ban/jail.d/meetrmail.conf
cp -f conf/fail2ban/filter.d/* /etc/fail2ban/filter.d/

# On first installation, the log files that the jails look at don't all exist.
# e.g. the roundcube error log isn't created until someone logs into Roundcube
# for the first time. fail2ban does not skip a jail whose log file is missing --
# it refuses to start at all ("Have not found any log file for sshd jail"), and
# a box where fail2ban silently failed to start has no brute-force protection
# whatsoever.
#
# rsyslog creates these on first write, which for auth.log means "when somebody
# first logs in over ssh" -- i.e. possibly after this point. So create the ones
# rsyslog owns now, with rsyslog's own permissions, and let it append to them.
# Later setup scripts create the application logs, and fail2ban gets another
# restart at the very end of setup.
for logfile in /var/log/auth.log /var/log/syslog /var/log/mail.log; do
	if [ ! -f "$logfile" ]; then
		install -m 640 -o syslog -g adm /dev/null "$logfile"
	fi
done
mkdir -p /var/log/nginx
if [ ! -f /var/log/nginx/access.log ]; then
	install -m 640 -o www-data -g adm /dev/null /var/log/nginx/access.log
fi

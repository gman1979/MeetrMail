...
# nginx configuration at /meetrmail-mobileconfig.
mkdir -p /var/lib/meetrmail
chmod a+rx /var/lib/meetrmail
cat conf/ios-profile.xml \
	...
	| sed "s/UUID4/$(cat /proc/sys/kernel/random/uuid)/" \
	 > /var/lib/meetrmail/mobileconfig.xml
chmod a+r /var/lib/meetrmail/mobileconfig.xml
...
cat conf/mozilla-autoconfig.xml \
	| sed "s/PRIMARY_HOSTNAME/$PRIMARY_HOSTNAME/" \
	 > /var/lib/meetrmail/mozilla-autoconfig.xml
chmod a+r /var/lib/meetrmail/mozilla-autoconfig.xml

# Create a generic mta-sts.txt file which is exposed via the
# nginx configuration at /.well-known/mta-sts.txt
# more documentation is available on:
# https://www.uriports.com/blog/mta-sts-explained/
# default mode is "enforce". In /etc/meetrmail.conf change
...
cat conf/mta-sts.txt \
        ...
        | sed "s/PRIMARY_HOSTNAME/$PUNY_PRIMARY_HOSTNAME/" \
         > /var/lib/meetrmail/mta-sts.txt
chmod a+r /var/lib/meetrmail/mta-sts.txt

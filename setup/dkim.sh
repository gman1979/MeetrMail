...
# the SPF check itself when this is set.
...
#
# Rspamd runs its own SPF, DKIM and DMARC checks for scoring rather than reading
# this header (v76's SpamAssassin rules regex-matched it), but the header is
# still worth having: it is what a user or an admin reads when working out why a
# message was or was not trusted.
...
#
# Be careful: smtpd_milters is a single value, so every milter has to be listed
# together. setup/rspamd.sh runs after this script and rewrites the line to add
# Rspamd's milter on :11332 after these two. If you change the value here,
# change it there too.
#

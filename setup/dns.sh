...
# Do not edit. Overwritten by MeetrMail setup.
server:
  hide-version: yes
...
# the zones.conf file written out by our management daemon. It has to exist
# before nsd-checkconf runs below, even while it is still empty.
...
# Check the configuration we just generated before anything depends on it.
#
# v76 targeted nsd 4.3.9; noble ships 4.8.0. Five minor releases of validation
# tightening sit in between, and nsd will refuse to start on a directive it no
# longer accepts. nsd-checkconf says so immediately and precisely, which is a
# great deal better than finding out from a failed `service nsd start` three
# scripts later.
if ! nsd-checkconf /etc/nsd/nsd.conf; then
	echo
	echo "The nsd configuration this script generated is not valid for the version of"
	echo "nsd installed here ($(nsd -v 2>&1 | head -n1)). The errors are above."
	echo
	exit 1
fi
...
cat > /etc/cron.daily/meetrmail-dnssec << EOF;
#!/bin/bash
# MeetrMail
# Re-sign any DNS zones with DNSSEC because the signatures expire periodically.
$PWD/tools/dns_update
EOF
chmod +x /etc/cron.daily/meetrmail-dnssec

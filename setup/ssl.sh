...
if [ ! -f "$STORAGE_ROOT/ssl/ssl_certificate.pem" ]; then
	...
	#
	# Note the subjectAltName. Every current TLS client ignores the CN and
	# requires the hostname in a SAN extension, so a CN-only self-signed
	# certificate is rejected outright rather than merely warned about. That
	# makes it impossible to test HTTPS against this certificate at all, which
	# matters both in sandbox mode and on a live box in the window before
	# certbot has issued a real certificate.
	CSR=/tmp/ssl_cert_sign_req-$$.csr
	CSR_EXT=/tmp/ssl_cert_sign_req-$$.ext
	cat > $CSR_EXT <<EOF;
subjectAltName = DNS:$PRIMARY_HOSTNAME
...
EOF
	...
	hide_output \
	openssl x509 -req -days 365 -extfile $CSR_EXT \
...
cat > /etc/cron.daily/meetrmail-ssl-cleanup << EOF;
#!/bin/bash
# MeetrMail
# Cleanup expired SSL certificates
$(pwd)/tools/ssl_cleanup
EOF
chmod +x /etc/cron.daily/meetrmail-ssl-cleanup

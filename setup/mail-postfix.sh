...
#
# Local mail is delivered straight to Dovecot over LMTP. Spam filtering happens
# during the SMTP transaction instead, in Rspamd's milter (see setup/rspamd.sh),
# so there is no longer a filtering hop in the delivery path. v76 sent mail to
# spampd first, which then passed it to Dovecot for
...
#
# Note: v76 also installed `postgrey` here for greylisting. Rspamd's greylist
# module does that job now -- see setup/rspamd.sh -- so postgrey is gone and
# setup/rspamd.sh purges it from boxes that have it.
...
tools/editconf.py /etc/postfix/main.cf \
	...
	smtpd_banner="\$myhostname ESMTP Hi, I'm a MeetrMail (Ubuntu/Postfix; see https://meetrmail.net/)" \
...
# Deliver local mail to Dovecot's LMTP listener on port 10026.
#
# setup/rspamd.sh sets this too, and is the script that owns the value now that
# spam filtering is a milter rather than a delivery hop. It is set here as well
# so that a box is never left pointing at v76's spampd port if only part of
# setup runs.
...
# * `check_policy_service inet:127.0.0.1:12340`: Dovecot's quota-status service, so that mail
#   for a mailbox that is already full is refused at SMTP time rather than accepted and bounced.
#
# Greylisting is no longer a policy service here. v76 ran postgrey on
# 127.0.0.1:10023 and listed it in this chain; Rspamd's greylist module does it
# from the milter instead, which means one fewer daemon and one shared Redis
# for greylisting, Bayes and fuzzy hashes. See setup/rspamd.sh.
#

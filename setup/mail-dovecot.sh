...
#
# imap_sieve is what lets setup/rspamd.sh hook "user moved a message into or out
# of the Spam folder" and retrain the spam filter from it. Both plugins are
# listed on one line because inside a `protocol imap` block $mail_plugins
# expands to the *global* setting, so a second assignment would discard the
# first rather than add to it.
if ! grep -q "mail_plugins.* imap_quota" /etc/dovecot/conf.d/20-imap.conf; then
  sed -i "s/\(mail_plugins =.*\)/\1\n  mail_plugins = \$mail_plugins imap_quota imap_sieve/" /etc/dovecot/conf.d/20-imap.conf
fi

# configure stuff for quota support
#
# This file is written every run rather than only when it is missing its
# contents, which is what v76 did. A generated config that is only ever created
# once can never be corrected: a fix made here would reach new installs and
# silently skip every existing box, including the one you just fixed it on.
...
# The quota policy service Postfix asks before accepting mail for a mailbox
# that is already full. Bound to loopback: Postfix only ever connects to
# 127.0.0.1:12340, and left on 0.0.0.0 (as v76 had it) anyone on the internet
# can ask it about an address and learn from the answer whether that mailbox
# exists.
...
# on port 10026, which is where Postfix delivers local mail
# (virtual_transport, see setup/mail-postfix.sh).
#
...
# as spam by Rspamd into the user's Spam folder.
#

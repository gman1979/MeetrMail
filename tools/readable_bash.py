...
<html>
    ...
    <div class="container">
      <div class="row intro">
        ...
        <p>This document is generated automatically from <a href="https://meetrmail.net">MeetrMail</a>&rsquo;s setup script <a href="https://github.com/meetrmail/meetrmail">source code</a>.</p>
        <hr>
      </div>
    </div>
...
class OtherLine(Grammar):
	...
	def value(self):
		...
		if "source /etc/meetrmail.conf" in self.string: return ""
		return "<pre class='shell'><div>" + recode_bash(self.string.strip()) + "</div></pre>\n"
...
class BashScript(Grammar):
	...
	@staticmethod
	def parse(fn):
		if fn in {"setup/functions.sh", "/etc/meetrmail.conf"}: return ""
		...
		return v.replace("`pwd`",  "<code><b>/path/to/meetrmail</b></code>")
